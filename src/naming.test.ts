...
describe('buildFileTree — title sanitization', () => {
  ...
  it('removes commas', () => {
    const node = buildFileTree(
      leaf('Regions, Availability Zones, and Local Zones', 'a.html'),
      '/out'
    );
...
import { fileNodeToSerial } from './naming.ts';
import type { TocNodeFile } from './providers/types.ts';

describe('fileNodeToSerial — leaf', () => {
  it('includes title and href, omits contents', () => {
    const node: TocNodeFile = {
      kind: 'leaf',
      title: 'Welcome',
      href: 'Welcome.html',
      filePath: '/out/Guide/Welcome.md',
    };
    const serial = fileNodeToSerial(node, '/out/Guide');
    expect(serial.title).toBe('Welcome');
    expect(serial.href).toBe('Welcome.html');
    expect(serial.contents).toBeUndefined();
  });

  it('uses relative filePath from base', () => {
    const node: TocNodeFile = {
      kind: 'leaf',
      title: 'Step 1',
      href: 'step1.html',
      filePath: '/out/Guide/Step-1.md',
    };
    const serial = fileNodeToSerial(node, '/out/Guide');
    expect(serial.filePath).toBe('./Step-1.md');
  });

  it('omits href when null', () => {
    const node: TocNodeFile = { kind: 'leaf', title: 'Empty', href: null, filePath: null };
    const serial = fileNodeToSerial(node, '/out');
    expect(serial.href).toBeUndefined();
    expect(serial.filePath).toBeUndefined();
  });
});

describe('fileNodeToSerial — branch', () => {
  it('includes nested contents', () => {
    const node: TocNodeFile = {
      kind: 'branch',
      title: 'DB Instances',
      href: null,
      dirPath: '/out/Guide/DB-Instances',
      filePath: null,
      children: [
        {
          kind: 'leaf',
          title: 'Overview',
          href: 'overview.html',
          filePath: '/out/Guide/DB-Instances/01-Overview.md',
        },
      ],
    };
    const serial = fileNodeToSerial(node, '/out/Guide/DB-Instances');
    expect(serial.contents).toHaveLength(1);
    expect(serial.contents![0]!.title).toBe('Overview');
    expect(serial.contents![0]!.filePath).toBe('./01-Overview.md');
  });

  it('omits contents key when branch has no children', () => {
    const node: TocNodeFile = {
      kind: 'branch',
      title: 'Empty Section',
      href: null,
      dirPath: '/out/Guide/Empty',
      filePath: null,
      children: [],
    };
    const serial = fileNodeToSerial(node, '/out/Guide/Empty');
    expect(serial.contents).toBeUndefined();
  });

  it('includes self-page filePath for branch with href', () => {
    const node: TocNodeFile = {
      kind: 'branch',
      title: 'DB Instances',
      href: 'db.html',
      dirPath: '/out/Guide/DB-Instances',
      filePath: '/out/Guide/DB-Instances/00-DB-Instances.md',
      children: [],
    };
    const serial = fileNodeToSerial(node, '/out/Guide/DB-Instances');
    expect(serial.href).toBe('db.html');
    expect(serial.filePath).toBe('./00-DB-Instances.md');
  });
});
